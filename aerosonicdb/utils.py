...
def load_flatten_env_test_data(data_path, json_base, target_label):
    ...
    for n in range(1, 7):
        ...
        if n == 1:
            x_test, y_test = load_flatten_test_data(data_path=full_path, target_label='class_label', print_msg=False)

        else:
            X, y = load_flatten_test_data(data_path=full_path, target_label='class_label', print_msg=False)
