...
def download(target_path=download_path):
    ...
    env_map_csv = (
        "https://zenodo.org/record/10215080/files/environment_class_mappings.csv"
    )
    raw_env_map = (
        "https://zenodo.org/record/10215080/files/environment_mappings_raw.csv"
    )
    ...
    if not os.path.exists(target_path):
        ...
        urllib.request.urlretrieve(
            license_txt, os.path.join(target_path, "LICENSE.txt")
        )
        ...
        with zipfile.ZipFile(
            os.path.join(target_path, "env_audio.zip"), "r"
        ) as zip_ref:
            zip_ref.extractall(target_path)
