...
def run_cv(rand_seed=0):
    ...
    build = build_model(X)
    model = KerasClassifier(model=build, epochs=50, batch_size=216, random_state=rand_seed, verbose=2)
...
def train_save_model(dir_path=output_path,
    ...
    # train model
    model.fit(X_train, y_train, validation_data=(X_val, y_val), batch_size=216, epochs=50)
